...
test("server-renders the Marvin portfolio map", async () => {
  ...
  assert.match(html, /AI systems I built to solve real engineering problems\./);
  ...
  assert.match(html, /The work that shaped how I build\./);
  assert.match(html, /Before moving into AI engineering, I spent years supporting production SQL Server environments/);
  ...
  assert.match(html, /<p class="credential-issuer">Anthropic<\/p>/);
  assert.match(html, /Demonstrates practical understanding of Claude workflows, prompting, configuration, tool use, and responsible AI system usage\./);
  ...
  assert.match(html, /<p>2026<\/p><h3>Bachelor of Science in Information Technology<\/h3>/);
  assert.doesNotMatch(html, /EXPECTED 2026/);
  ...
  for (const capability of ["AI APIs &amp; Backends", "Agent Workflows", "Reliability &amp; Evaluation", "Database &amp; Systems Foundations"]) assert.match(html, new RegExp(capability));
  assert.match(html, /The systems and skills I bring together\./);
  assert.doesNotMatch(html, /04 · CAPABILITIES/);
...
test("renders the portfolio navigation and main sections", async () => {
  ...
  assert.ok(topnav);
  assert.match(topnav, /href="https:\/\/github\.com\/marvinjbb" target="_blank" rel="noopener noreferrer">GitHub<\/a>/);
  assert.match(topnav, /LinkedIn/);
  assert.match(topnav, /Let(?:&#x27;|')s Connect/);
  for (const duplicate of ["Projects", "Experience", "What I Build", "Credentials", "Education", "Articles"]) assert.doesNotMatch(topnav, new RegExp(`>${duplicate}<`));
  ...
  for (const capability of ["AI APIs &amp; Backends", "Agent Workflows", "Reliability &amp; Evaluation", "Database &amp; Systems Foundations"]) assert.match(html, new RegExp(`<h3>${capability}<\\/h3>`));
  assert.doesNotMatch(html, /class="capability-strengths"/);
